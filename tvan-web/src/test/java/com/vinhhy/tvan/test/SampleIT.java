...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "tvan-ear.ear");

        // Current directory is the root of the "tvan-web" project. Go up one level
        // and enter the "tvan-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../tvan-ejb/target/tvan-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "tvan-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/tvan-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "tvan-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("com.vinhhy.tvan.test");
